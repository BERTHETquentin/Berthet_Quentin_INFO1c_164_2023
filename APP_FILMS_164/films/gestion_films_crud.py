    ...
    if request.method == "POST":
        try:
            if form_add_film.validate_on_submit():
                ...
                prenom_client_add = form_add_film.prenom_film_add_wtf.data

                valeurs_insertion_dictionnaire = {"value_nom_film": nom_film_add,
                                                  "Value_prenom_client": prenom_client_add}
                print("valeurs_insertion_dictionnaire ", valeurs_insertion_dictionnaire)

                strsql_insert_film = """INSERT INTO t_client (ID_Client,Nom_Client,Prenom_Client) 
                                        VALUES (NULL,%(value_nom_film)s,%(Value_prenom_client)s) """
...
def film_update_wtf():
    ...
    try:
        ...
        if form_update_film.validate_on_submit():
            ...
            prenom_film_update = form_update_film.prenom_film_update_wtf.data

            valeur_update_dictionnaire = {"value_id_film": id_film_update,
                                          "value_nom_film": nom_film_update,
                                          "value_prenom_film": prenom_film_update
                                          }
            print("valeur_update_dictionnaire ", valeur_update_dictionnaire)

            str_sql_update_nom_film = """UPDATE t_client SET Nom_Client = %(value_nom_film)s WHERE ID_Client = %(value_id_film)s"""
            ...
            # Opération sur la BD pour récupérer "id_film" et "intitule_genre" de la "t_produit"
            str_sql_id_film = "SELECT * FROM t_clientWHERE ID_Client = %(value_id_film)s"
...
def film_delete_wtf():
    ...
    try:
        ...
        if form_delete_film.submit_btn_del_film.data:
            ...
            str_sql_delete_fk_film_genre = """DELETE  FROM t_client WHERE FK_Client = %(value_id_film)s"""
            str_sql_delete_film = """DELETE FROM t_client WHERE ID_Client = %(value_id_film)s"""
        ...
        if request.method == "GET":
            ...
            str_sql_genres_films_delete = """SELECT * FROM t_client WHERE ID_Client = %(value_id_film)s"""
